...
describe Upsteem::Deploy::Tasks::FeatureBranchSyncing do
  ...
  def expect_merge_commit_creation_and_further
    expect_to_receive_exactly_ordered(1, git_service, :create_merge_commit, "origin/master")
    expect_to_receive_exactly_ordered(
      1, git_service, :commit, "Sync #{feature_branch} with origin/master", all: true
    )
    expect_to_receive_exactly_ordered(1, git_service, :push, "origin", feature_branch)
  ...
  describe "#run" do
    before do
      ...
      expect_to_receive_exactly_ordered(1, git_service, :checkout, feature_branch)
